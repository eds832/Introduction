...
public class SuperService {
	...
	private static final RowMapper cabRowMapper = (rs, rowNumber) -> new Cab(rs.getDate("manufacture_year"),
			rs.getString("car_make"), rs.getString("licence_plate"), rs.getInt("capacity"),
	...
	public Cab findByYear(int manufactureYear) {
		...
		List<Cab> result = namedParameterJdbcTemplate.query(
				"SELECT * FROM `cab` WHERE YEAR(`cab`.`manufacture_year`) = :manufactureYear", namedParameters,
				cabRowMapper);
